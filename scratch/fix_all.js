...
html = html.replace(modalFix1, modalFix1_new);
...
html = html.replace(modalFix2, modalFix2_new);
...
html = html.replace(adminHeaderOld, adminHeaderNew);
...
html = html.replace(musicoHeaderOld, musicoHeaderNew);

// 4. Inject logic for admin dropdown
const adminLogic = `      // Lógica de Opciones (Admin Dropdown)
...
`;
html = html.replace("      // Nodos DOM globales", adminLogic + "\n      // Nodos DOM globales");
...
html = html.replace("      const btnUpdatePwa = document.getElementById('btn-musico-update');", musicoLogoutLogic + "\n      const btnUpdatePwa = document.getElementById('btn-musico-update');");
