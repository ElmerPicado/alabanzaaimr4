        ...
        reader.onload = async (event) => {
          try {
            ...
            if (!Array.isArray(songsArray) || songsArray.length === 0) {
              alert("El archivo JSON debe contener un arreglo de canciones.");
              return;
            }

            if (!confirm(`¿Estás seguro de que quieres importar ${songsArray.length} canciones al repertorio?`)) {
    ...
    window.dangerouslyDeleteAllSongs = async function () {
      if (!confirm("⚠️ ATENCIÓN: Esto borrará permanentemente TODAS las canciones de la biblioteca y sus letras. ¿Estás seguro?")) return;
      if (!confirm("¿Seguro al 100%? Esta acción NO se puede deshacer.")) return;
      ...
      alert(`¡Completado! Se borraron ${count} canciones.`);
    };

    window.dangerouslyFixAllCorruptedTexts = async function () {
      if (!confirm("⚠️ ATENCIÓN: Esto buscará y corregirá caracteres corruptos (Mojibake) en TODAS las canciones y letras de la base de datos. ¿Estás seguro?")) return;
    ...
    function generarMensajeTexto(dataServicio) {
      ...
      if (dataServicio.nota_adicional && dataServicio.nota_adicional.trim() !== "") {
        msg += `<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="margin-right:4px;"><path d="M21.174 6.812a1 1 0 0 0-3.986-3.987L3.842 16.174a2 2 0 0 0-.5.83l-1.321 4.352a.5.5 0 0 0 .623.622l4.353-1.32a2 2 0 0 0 .83-.497z"></path><path d="m15 5 4 4"></path></svg> *NOTA DEL LÍDER:*\n_${dataServicio.nota_adicional}_\n\n`;
      }
      ...
      msg += `<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="margin-right:4px;"><path d="M9 18V5l12-2v13"></path><circle cx="6" cy="18" r="3"></circle><circle cx="18" cy="16" r="3"></circle></svg> *REPERTORIO SELECCIONADO:*\n`;

      // Adoración primero
      msg += `*<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="margin-right:4px;"><path d="M12 20a8 8 0 1 0 0-16 8 8 0 0 0 0 16Z"></path><path d="M12 14a2 2 0 1 0 0-4 2 2 0 0 0 0 4Z"></path><path d="M12 2v2"></path><path d="M12 22v-2"></path><path d="m17 20.66-1-1.73"></path><path d="M11 10.27 7 3.34"></path><path d="m20.66 17-1.73-1"></path><path d="m3.34 7 1.73 1"></path><path d="M14 12h8"></path><path d="M2 12h2"></path><path d="m20.66 7-1.73 1"></path><path d="m3.34 17 1.73-1"></path><path d="m17 3.34-1 1.73"></path><path d="m11 13.73-4 6.93"></path></svg> ADORACIÓN (Lentas):*\n`;
    ...
    function renderizarComponentesServicio(dataServicio) {
      ...
      const filteredSongs = cacheSongs.filter(s => removeAccents(s.nombre || "").toLowerCase().includes(filterText) || removeAccents(s.artista || "").toLowerCase().includes(filterText));
      ...
      filteredSongs.forEach(s => {
        ...
        const isAlabanza = s.tipo === 'alabanza';
        const cardBorder = isAlabanza
          ? 'border-left: 4px solid var(--teal) !important; border-color: rgba(78, 205, 196, 0.25) !important;'
          : 'border-left: 4px solid var(--purple) !important; border-color: rgba(155, 143, 255, 0.25) !important;';
        const cardBg = isAlabanza
          ? 'background: rgba(78, 205, 196, 0.04) !important;'
          : 'background: rgba(155, 143, 255, 0.04) !important;';
        ...
        item.className = "musico-song-card";
        item.style.cssText = `${cardBg} ${cardBorder} margin-bottom: 8px !important; border-radius: var(--radius-sm) !important; border-bottom: 1px solid var(--border) !important;`;
        ...
        item.innerHTML = `
          <!-- Extremo Izquierdo: Checkbox -->
          <div style="display:flex; align-items:center; gap:8px;">
            <div class="song-check ${isSelected ? 'checked' : ''} ${currentUserRole !== 'lider' ? 'song-locked' : ''}" style="margin: 0;"></div>
          </div>
          
          <!-- Centro: Info -->
          <div class="song-card-info" style="margin-left: 4px;">
            <div class="song-card-name" style="font-weight: 600;">${s.nombre}</div>
            <div class="song-card-artist" style="font-size:11px; color: var(--text2); display:flex; align-items:center; gap:6px; flex-wrap:wrap; margin-top:2px;">
              <span>${s.artista}</span>
              <span>•</span>
              <span>Base: <b style="color:var(--gold);">${s.tonoBase}</b></span>
            </div>
          </div>
          
          <!-- Extremo Derecho: Tono si está seleccionado -->
    ...
    document.getElementById('form-add-song').addEventListener('submit', async (e) => {
      ...
      if (!editingSongId) {
        ...
        if (existe) {
          alert(`Ya existe "${nombre}" de ${artista} en el catálogo.\n\nNo se puede agregar duplicado.`);
          return;
        }
      }
    ...
    document.getElementById('btn-fetch-lacuerda').addEventListener('click', async () => {
      ...
      try {
        ...
        if (odesMatch) {
          ...
          if (chords.length > 0) {
            tonoBase = normalizarTono(chords[0]);
          }
        } else {
          // Fallback al texto de la página
          const bodyText = doc2.body?.innerText || html;
          const tonoMatch = bodyText.match(/[Tt]ono[:\s]+([A-Ga-gDdoReeMiiFaaSollLasiSI#bm]+)/i);
          if (tonoMatch) {
            tonoBase = normalizarTono(tonoMatch[1]);
          } else if (letra) {
            // Intentar adivinar por el primer acorde
            const chordRegex = /\b(DO|RE|MI|FA|SOL|LA|SI|[CDEFGAB])[#bm]*\b/i;
            const firstChordMatch = letra.match(chordRegex);
            if (firstChordMatch) {
              tonoBase = normalizarTono(firstChordMatch[0]);
            }
          }
        }

        tonoBase = normalizarTono(tonoBase);
    ...
    let _currentZoom = window.innerWidth >= 768 ? 32 : 28; // default tablet: 32, mobile: 28
    ...
    window.abrirModalLetraGlobal = async function (cancionId, playlistContext, forceModal = false) {
      detenerAutoscroll();
    ...
    function renderSelectedSongsAdmin(dataServicio) {
      ...
      if (allSelected.length === 0) {
        cont.innerHTML = `<p style="color: var(--text3); font-size: 13px; text-align: center; padding: 20px; background: var(--bg3); border: 1px solid var(--border); border-radius: var(--radius); margin: 0;">No hay canciones seleccionadas aún.</p>`;
        return;
      }
    ...
    if (btnAddSelected) {
      btnAddSelected.addEventListener('click', async () => {
        ...
        window._selectedSongsToAdd.forEach(songId => {
          ...
          if (s) {
            updates[`cancionesSeleccionadas.${songId}`] = s.tonoBase;
            updates[`cancionesSeleccionadasTipos.${songId}`] = type;
            currentOrder.push(songId);
          }
        });
        updates['ordenCanciones'] = currentOrder;
    ...
    function setupProfilePicUpload() {
      ...
      if (leaderAvBtn) leaderAvBtn.onclick = triggerUpload;
